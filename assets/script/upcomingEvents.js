...
createApp({
    ...
    created(){
        ...
        .then(data =>{
            ...
            this.filterEventsUpcoming = this.arrayEvents.filter(event => event.date > data.currentDate)
    ...
    computed:{
        ...
        filter(){
            this.filterEvents = this.filterEventsUpcoming.filter(event => event.name.toLowerCase().includes(this.inputSearch.toLowerCase())
